...
- constructor(selector: string, events: IEvents) Конструктор принимает в качестве параметров селектор, по которому в размете страницы будет идентифицировано модальное окно и экземпляр класса `EventEmitter` для возможности инициации событий.
...
- submitButton: HTMLButtonElement - DOM-элемент кнопки добавления товара в корзину;
- handleSubmit: Function - функция, отвечающая за добавления товара в корзину.
...
Поля класса:
- submitButton: HTMLButtonElement - DOM-элемент кнопки оформления заказа;
...
- handleSubmit: Function - функция, отвечающая за открытие модального окна оформление заказа.
...
- buttonChoicePayment: HTMLButtonElement - DOM-элемент кнопки выбора способа оплаты, в которой должен быть активирован класс `button_alt-active`;
