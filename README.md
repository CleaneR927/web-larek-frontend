...
```
2. Данный интерфейс описывает данные, которые ожидается получить с сервера.
```
...
```
4. Интерфейс для хранения списка товаров и общей суммы заказа.
```
interface IBasket {
	items: HTMLElement[];
	totalCost: number;
...
```
5. Интерфейс для хранения идентификатора созданного заказа и количество списанной валюты.
```
interface IOrderResult {
  total: number;
}
```
6. Интерфейс для получение карточек с сервера, их вывода на страницу. Добавления и удаления карточек из корзины. Работу валидации формы и обновление данных.
```
interface IAppData {
  basket: IProduct[];
  products: IProduct[];
...
```
7. Интерфейс предназначен для получения данных, чтобы в дальнейшем по ним производить запрос на сервер.
```
interface IApi {
  baseUrl: string;
...
```
8. Интерфейс предназначен формирования модального окна.
```
...
Код приложения разделен на три слоя, согласно парадигме MVP:
...
- сеттеры valid и errors задают состояние компонента;
