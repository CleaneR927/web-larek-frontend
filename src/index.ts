...
import { IApiResponse, CustomerContacts, CustomerForm, CustomerOrder, IApi, IProduct} from './types';
...
const contacts = new UserContactDetails(cloneTemplate(contactsTemplate), events);
...
api
  .get('/product')
  .then((cards: IApiResponse) => {
      appData.setProducts(cards.items as IProduct[]);
    })
  .catch((error) => {
...
events.on('orderInput:change', (data: { field: keyof CustomerForm, value: string }) => {
...
events.on('contacts:submit', () => {
  api.post('/order', appData.customerOrder)
    .then((res) => {
